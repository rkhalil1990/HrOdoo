...
class hr_employee_ref(osv.Model):
    _name = "hr.employee.ref"
    _description = "Employee Ref"
    _columns = {
        'name': fields.char("Name", required=True),
        'emp_id': fields.many2one("hr.employee", "Emp Id", required=True, ondelete='cascade'),
        'telephone_no': fields.char("Telephone No.", required=True),
        'relation': fields.char("Relation"),
        'email_id': fields.char("Email Id"),
        'address': fields.char("Address"),
    }

    @api.onchange('telephone_no')
    def onchange_mobile(self):
        for rec in self:
            if not rec.telephone_no:
                return {}

            if not rec.telephone_no.isdigit():
...
class hr_employee_family(osv.Model):
    ...
    _columns = {
        ...
        'staying_at': fields.char('Staying At'),
    }
...
class hr_employee(osv.osv):
    ...
    _columns = {
        ...
        'ref_ids': fields.one2many('hr.employee.ref', 'emp_id', 'Reference Ids', copy=True),
        'family_ids': fields.one2many('hr.employee.family', 'emp_id', 'Family Ids', copy=True),
        'x_ration_card_id': fields.char('Ration Card'),
        'x_driving_license_id': fields.char('Driving-License'),
        'x_aadhar_id': fields.char('Aadhar Number'),
        'x_pan_id': fields.char('PAN Number'),
        'x_personal_bank_name': fields.char('Bank Name'),
        'x_salary_bank_name': fields.char('Bank Name'),
        'x_personal_ifsc_code': fields.char('IFSC Code'),
        'x_salary_ifsc_code': fields.char('IFSC Code'),
        'x_mother_name': fields.char("Mother’s Name"),
        'x_father_name': fields.char("Father/Husband Name"),
        'x_previous_occupation': fields.char("Previous Occupation"),
        'x_total_family_members': fields.integer("Total Family Members"),
        'x_house_rent': fields.integer('House Rent'),
        'x_health_isues': fields.text('Health Issues (If any)'),
        'x_date_of_join': fields.date('Date of joining'),
        'x_bdf_sub_date': fields.date('BDF Submission Date'),
        'x_personal_acc_number': fields.char('Account Number'),
        'x_salary_acc_number': fields.char('Account Number'),
    }
    ...
    x_aadhar_id_image = openerp.fields.Binary("Aadhar Id Image", attachment=True, help="Aadhar Id Image")
    x_aadhar_image_medium = openerp.fields.Binary("Aadhar Medium-sized photo", \
        compute='_compute_aadhar_images', inverse='_inverse_aadhar_medium', store=True, \
        attachment=True, help="Aadhar Medium-sized photo")

    x_passport_image = openerp.fields.Binary("Passport Image", attachment=True, help="Passport Image")

    x_pan_image = openerp.fields.Binary("PAN Card Image", attachment=True, help="PAN Card Image")

    x_driving_license_image = openerp.fields.Binary("DL Image", attachment=True, help="DL Image")

    x_rationcard_id_image = openerp.fields.Binary("Ration Id Image", attachment=True, help="Ration Id Image")
    ...
    @api.depends('x_aadhar_id_image')
    def _compute_aadhar_images(self):
        for rec in self:
            rec.x_aadhar_image_medium = tools.image_resize_image_medium(rec.x_aadhar_id_image)

    def _inverse_aadhar_medium(self):
        for rec in self:
            rec.x_aadhar_id_image = tools.image_resize_image_big(rec.x_aadhar_image_medium)
